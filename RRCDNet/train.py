...
# 1.日志配置
def setup_logger():
    os.makedirs("RRCDNet-logs", exist_ok=True)
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = f"RRCDNet-logs/training_{current_time}.log"
...
# 3. 定义模型
class RRCDNet(nn.Module):
    def __init__(self):
        super(RRCDNet, self).__init__()

        # 右网络（标准 CNN）
        self.right_net = nn.Sequential(
            nn.Conv1d(1, 64, 3, padding=1), nn.BatchNorm1d(64), nn.ReLU(),
            *[nn.Sequential(nn.Conv1d(64, 64, 3, padding=1), nn.BatchNorm1d(64), nn.ReLU()) for _ in range(15)],
        ...
        self.left_net = nn.Sequential(
            nn.Conv1d(1, 64, 3, padding=1), nn.BatchNorm1d(64), nn.ReLU(),
            *[nn.Sequential(nn.Conv1d(64, 64, 3, dilation=2, padding=2), nn.ReLU()) for _ in range(7)],
            nn.Conv1d(64, 64, 3, padding=1), nn.BatchNorm1d(64), nn.ReLU(),
            *[nn.Sequential(nn.Conv1d(64, 64, 3, dilation=2, padding=2), nn.ReLU()) for _ in range(6)],
            nn.Conv1d(64, 1, 3, padding=1)
        )
    ...
    def forward(self, x):
        right_out = self.right_net(x)
        left_out = self.left_net(x)
        return x - (right_out + left_out) / 2  # 残差学习
    ...
    def _initialize_weights(self):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv1d):
                # 使用 Kaiming 初始化方法初始化卷积层权重
                init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                if m.bias is not None:
                    init.constant_(m.bias, 0)  # 初始化偏置为 0
...
LR = 0.0003
BATCH_SIZE = 32
SAVE_PATH = "RRCDNet-saved_model/RRCDNet_best.pth"
...
training_metadata = {
    ...
    "model_architecture": str(RRCDNet())
}
