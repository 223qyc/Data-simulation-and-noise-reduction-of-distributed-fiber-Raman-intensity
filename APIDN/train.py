...
# 1.日志配置
def setup_logger():
    os.makedirs("APIDN-logs", exist_ok=True)
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = f"APIDN-logs/training_{current_time}.log"
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        avg_out = torch.mean(x, dim=1, keepdim=True)  # shape: [B, 1, L]
        max_out, _ = torch.max(x, dim=1, keepdim=True)
        x_cat = torch.cat([avg_out, max_out], dim=1)  # shape: [B, 2, L]
        x_out = self.conv(x_cat)
        return self.sigmoid(x_out)  # shape: [B, 1, L]
...
class APIDN(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv_out(x + identity)  # 加上整体残差再输出
        return out
...
LR = 0.001
BATCH_SIZE = 16
SAVE_PATH = "APIDN-saved_model/APIDN_best.pth"
